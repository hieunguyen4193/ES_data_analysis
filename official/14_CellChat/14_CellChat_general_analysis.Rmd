---
title: "CellChat analysis, one sample `r params$sample.id`"
author:
...
path.to.save.output <- file.path("/media/hieunguyen/CRC1382H/CRC1382/outdir/EStange_20240411_SeuratV5/data_analysis/14_output/all.samples/", sample.id)
...
data.matrix <- GetAssayData(object = s.obj, assay = "SCT", slot = "data")
```
...
meta.data <- s.obj@meta.data %>% 
  ...
  rowwise() %>%
  mutate(cluster = sprintf("cluster_%s", seurat_clusters)) %>%
  column_to_rownames("barcode")
num.cluster <- length(unique(s.obj$seurat_clusters))

all.cluster.idx <- to_vec (for (cluster.id in unique(s.obj$seurat_clusters)) str_replace(cluster.id, "cluster_", "")) %>% as.numeric()

meta.data$cluster <- factor(meta.data$cluster, levels = to_vec ( for (i in sort(all.cluster.idx)) sprintf("cluster_%s", i) ))
cellchat <- createCellChat(object = ^, meta = meta.data, group.by = "cluster")
