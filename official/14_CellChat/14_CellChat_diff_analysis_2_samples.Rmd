...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
library(uwot)
path.to.main.src.project <- "/home/hieunguyen/CRC1382/src_2023/EStange/CRC1382_EStange_projects/pipeline_all_samples_deep"
# path.to.main.src.project <- "/home/uk104163/src_2023/EStange/CRC1382_EStange_projects/pipeline_all_samples_deep"
source(file.path(path.to.main.src.project, "report_templates", "09_CellChat", "netEmbedding.modified.R"))

if (file.exists(file.path(path.to.09.output.CellChatCompare, sprintf("cellchat_merge_object_%s_vs_%s_%s.SimilarityPairwise.rds", sample1, sample2, filter10cells))) == FALSE){
  ...
  #> Classification learning of the signaling networks for datasets 1 2
  # Visualization in 2D-space
  saveRDS(cellchat, file.path(path.to.09.output.CellChatCompare, sprintf("cellchat_merge_object_%s_vs_%s_%s.SimilarityPairwise.rds", sample1, sample2, filter10cells)))
} else {
  cellchat <- readRDS(file.path(path.to.09.output.CellChatCompare, sprintf("cellchat_merge_object_%s_vs_%s_%s.SimilarityPairwise.rds", sample1, sample2, filter10cells)))
}
```
...
for (source.cluster in plt.clusters){
  ...
  if (num.NA.ligand == nrow(source.net.up) | num.NA.receptor == nrow(source.net.up)){
...
for (source.cluster in plt.clusters){
  ...
  if (num.NA.ligand == nrow(source.net.down) | num.NA.receptor == nrow(source.net.down)){
    gg1 <- ggplot() + ggtitle("No significant signaling signal to show!")
  } else {
...
##### SAVE R OBJECTS
#####----------------------------------------------------------------------#####
saveRDS(cellchat, file.path(path.to.09.output.CellChatCompare, sprintf("cellchat_merge_object_%s_vs_%s_%s.final.rds", sample1, sample2, filter10cells)))
```
