...
src.dir <- "07_filter_TCR_genes"
path.to.rmd <- file.path(path.to.main.src, src.dir, "07_data_analysis_after_subseting_and_integration.Rmd")
output_dir <- file.path(path.to.save.html, "07_output")
...
for (filter.mode in all.filter.cases){
  for (regression.mode in names(cell.cycle.features)){
    for (integration.case in names(integration.config)){
      output_file <- str_replace(basename(path.to.rmd), ".Rmd", sprintf(".%s.%s.%s.html", integration.case, regression.mode, filter.mode))
