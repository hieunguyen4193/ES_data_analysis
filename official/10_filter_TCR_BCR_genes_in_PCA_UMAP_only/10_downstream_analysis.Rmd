...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
FeaturePlot(object = s.obj, features = "AmbientRNA", label = TRUE, label.size = 8, reduction = "cca_UMAP")
```
...
for (sample.id in unique(s.obj$name)){
  ...
  p <- FeaturePlot(object = subset(s.obj, name == sample.id), features = "AmbientRNA", label = TRUE, label.size = 8, reduction = "cca_UMAP")
  print(p)
  cat("\n \n")
}
```
...
DimPlot(object = s.obj, reduction = "cca_UMAP", cells.highlight = high.ambientRNA.cells,
...
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE, fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.10.output, "high.ambientRNA.markers.rds")) == FALSE){
  s.obj <- PrepSCTFindMarkers(s.obj)
  high.ambientRNA.markers <- FindMarkers(object = s.obj, ident.1 = "yes", ident.2 = "no", group.by = "high.ambientRNA", assay = "SCT", test.use = "wilcox")
  ...
  saveRDS(high.ambientRNA.markers.raw, file.path(path.to.10.output, "high.ambientRNA.markers.raw.rds"))
  saveRDS(high.ambientRNA.markers, file.path(path.to.10.output, "high.ambientRNA.markers.rds"))
} else {
  high.ambientRNA.markers <- readRDS(file.path(path.to.10.output, "high.ambientRNA.markers.rds"))
  high.ambientRNA.markers.raw <- readRDS(file.path(path.to.10.output, "high.ambientRNA.markers.raw.rds"))
}
