...
run_07 <- function(outdir, 
  ...
  if (file.exists(file.path(path.to.10.output, "s8_output", "EStange_20240411_SeuratV5.output.s8.csv")) == FALSE){
    ...
    write.csv(data.frame(status = c("finished saving...")), 
              file.path(path.to.10.output, "s8_output", "EStange_20240411_SeuratV5.output.s8.csv"))
  } else {
    print(sprintf("integrated done. file at %s", file.path(path.to.10.output, "s8_output", "EStange_20240411_SeuratV5.output.s8.rds")))
  }
}
