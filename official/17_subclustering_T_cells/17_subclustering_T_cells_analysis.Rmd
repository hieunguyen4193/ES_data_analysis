---
title: "Analysis on the integrated dataset, integration: `r params$integration.case`, regression: `r params$regression.mode`, filter by `r params$filter.mode`, sub-cluster: `r params$subcluster.name`, `r params$tcell.sub.cluster` "
author:
...
tcell.sub.cluster <- params$tcell.sub.cluster 
...
path.to.s.obj <- file.path(path.to.17.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT))
...
if (file.exists(file.path(path.to.17.output, "DE_cluster_marker_genes.rds")) == FALSE){
  ...
  saveRDS(cluster.markers, file.path(path.to.17.output, "DE_cluster_marker_genes.rds"))
} else {
  cluster.markers <- readRDS(file.path(path.to.17.output, "DE_cluster_marker_genes.rds"))
}
```
