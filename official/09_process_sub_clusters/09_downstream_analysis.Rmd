...
path.to.s.obj <- file.path(path.to.09.output, "s8_output", sprintf("%s.output.s8.rds", PROJECT))
...
if (file.exists(file.path(path.to.09.output, "DE_cluster_marker_genes.rds")) == FALSE){
  ...
  saveRDS(cluster.markers, file.path(path.to.09.output, "DE_cluster_marker_genes.rds"))
} else {
  cluster.markers <- readRDS(file.path(path.to.09.output, "DE_cluster_marker_genes.rds"))
}
```
