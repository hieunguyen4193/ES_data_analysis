...
for (subcluster.name in c("b_cells", "lymphoids", "myeloids")){
  path.to.09.output <- file.path(path.to.main.output2, "09_output", integration.case, regression.mode, filter.mode, subcluster.name)
  dir.create(path.to.09.output, showWarnings = FALSE, recursive = TRUE)
  
  path.to.s.obj <- file.path(path.to.main.output2, "08_output", integration.case, regression.mode, filter.mode, sprintf("subcluster_%s.rds", subcluster.name))
