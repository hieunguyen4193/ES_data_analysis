---
title: "Preliminary analysis: `r params$sample.id`, Cell cycle regression mode: `r params$regression.mode`"
author:
...
# sample.id <- "d4_SPF"
# regression.mode <- "S_G2M_G1_scores"
...
path.to.05.output <- file.path(path.to.main.output, "05_output", sample.id)

s.obj <- readRDS(file.path(path.to.05.output, regression.mode, sprintf("%s.cellcycle_reg.rds", sample.id)))
...
# Cell cycle plots
## Before correcting cell cycle
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = s.obj, reduction = "CELLCYCLE_PCA_before", label = TRUE, group.by = "Phase", label.box = TRUE)
```

## After correcting cell cycle
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = s.obj, reduction = "CELLCYCLE_PCA_after", label = TRUE, group.by = "Phase", label.box = TRUE)
```
...
if (file.exists(file.path(path.to.05.output, "DE_cluster_marker_genes.rds")) == FALSE){
  ...
  saveRDS(cluster.markers, file.path(path.to.05.output, "DE_cluster_marker_genes.rds"))
} else {
  cluster.markers <- readRDS(file.path(path.to.05.output, "DE_cluster_marker_genes.rds"))
}
```
...
if (file.exists(file.path(path.to.05.output, sprintf("GEX_sample_%s_seurat_object.rds", sample.id))) == FALSE){
  saveRDS(object = s.obj, file = file.path(path.to.05.output, sprintf("GEX_sample_%s_seurat_object.rds", sample.id)))  
}
```
