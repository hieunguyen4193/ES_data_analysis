...
parser$add_argument("-i", "--input_row", action="store",
                    help="input row in the sample sheet to run monocle2")
...
dir.create(path.to.monocle2.output, showWarnings = FALSE, recursive = TRUE)

path.to.monocle.obj <- file.path(path.to.monocle2.input, 
                                    sprintf("%s.monocle2.rds", PROJECT))

if (file.exists(file.path(path.to.monocle2.output, "finished.csv")) == FALSE){
  print(sprintf("reading monocle2 prepared object from %s", path.to.monocle.obj))
  print(sprintf("output saved to %s", path.to.monocle2.output))
  ...
  write.csv(data.frame(stats = c("finished")), file.path(path.to.monocle2.output, "finished.csv"))
} else{
  print(sprintf("File output exists at %s", path.to.monocle2.output))
}
