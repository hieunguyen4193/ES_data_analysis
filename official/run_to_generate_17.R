...
src.dir <- "17_subclustering_T_cells"
path.to.rmd <- file.path(path.to.main.src, src.dir, "17_subclustering_T_cells_analysis.Rmd")
output_dir <- file.path(path.to.save.html, "17_output")
...
for (subcluster.name in c("T_cells")){
  for (tcell.sub.cluster in c("v1")){
    output_file <- str_replace(basename(path.to.rmd), ".Rmd", sprintf("%s.%s.%s.%s.%s.html", 
                                                                      subcluster.name, 
                                                                      integration.case, 
                                                                      regression.mode, 
                                                                      filter.mode,
                                                                      tcell.sub.cluster))
    print(sprintf("Working on %s - %s - %s - sub cluster: %s, T-cell subcluster: %s", 
                  filter.mode, 
                  regression.mode, 
                  integration.case, 
                  subcluster.name,
    ...
    if (file.exists(file.path(output_dir, output_file)) == FALSE){
      rmarkdown::render(input = path.to.rmd, 
                        ...
                        params = list(integration.case = integration.case,
                                      ...
                                      tcell.sub.cluster = tcell.sub.cluster))
    }      
  }
}
