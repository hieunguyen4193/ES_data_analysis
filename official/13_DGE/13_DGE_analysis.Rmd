...
Name of the input Seurat data object used in this analysis: 
`r basename(params$path.to.s.obj)`

Path:
`r params$path.to.s.obj`
...
# sample1 <- "d7_SPF"
# sample2 <- "d7_GF"
# path.to.s.obj <- "/media/hieunguyen/CRC1382H/CRC1382/outdir/EStange/CRC1382_EStange_projects/230605_220928_Stange/data_analysis/05_output/all_sobj_remove_d4_LPS.integrated.rds/recluster_VDJ/TRAV11.TRAJ18/reclustering_case_A_TRAV11.TRAJ18.neg.rds"
# foxp3 <- "notgiven"
...
# outdir <- "/media/outdir/EStange"
path.to.maindir <- sprintf("%s/CRC1382_EStange_projects/%s/data_analysis", outdir, run)

path.to.01.output <- file.path(path.to.maindir, "01_output")
path.to.02.output <- file.path(path.to.maindir, "02_output")
path.to.04.output <- file.path(path.to.maindir, "04_output")

path.to.06.output <- file.path(path.to.maindir, "06_output", 
                               str_replace(basename(path.to.s.obj), ".rds", ""), 
                               sprintf("%s_%s_Foxp3_%s", sample1, sample2, foxp3))
dir.create(path.to.06.output, showWarnings = FALSE, recursive = TRUE)

count.mtx.foxp3 <- GetAssayData(s.obj.original, slot = "counts", assay = "RNA")["Foxp3", ]

if (foxp3 == "positive"){
  foxp3.cells <- names(count.mtx.foxp3[count.mtx.foxp3 > 0])
  s.obj <- subset(s.obj.original, cells = foxp3.cells)
} else if (foxp3 == "zero") {
  foxp3.cells <- names(count.mtx.foxp3[count.mtx.foxp3 == 0])
  s.obj <- subset(s.obj.original, cells = foxp3.cells)
} else {
  s.obj <- s.obj.original
}
```
...
if (file.exists(file.path(path.to.06.output, savename)) == FALSE){
  de.markers <- hash()
  de.markers.raw <- hash()
  num.clusters <- length(unique(s.obj$seurat_clusters))
  
  for (cluster.id in unique(s.obj$seurat_clusters)){
    tmp.s.obj <- subset(s.obj, seurat_clusters == cluster.id)
  ...
  saveRDS(object = de.markers, file.path(path.to.06.output, savename))
  saveRDS(object = de.markers.raw, file.path(path.to.06.output, savename.raw))
} else {
  de.markers <- readRDS(file.path(path.to.06.output, savename))
  de.markers.raw <- readRDS(file.path(path.to.06.output, savename.raw))
}
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    p <- DoHeatmap(object = subset(s.obj, seurat_clusters == as.numeric(str_replace(cluster.id, "cluster_", ""))), features = c(top10up.genes$Gene, top10down.genes$Gene), group.by = "name", assay = "RNA")  
  }
  print(p)
  cat("\n \n")
}
...
### GO-ORA {.tabset}
```{r echo=FALSE, warning=FALSE, results='asis'}
if (file.exists(file.path(path.to.06.output, "all_ORA_GO_objects.single_cell.rds")) == FALSE){
  go.ora <- hash()
  ...
  saveRDS(all.ORA.GO.objects, file.path(path.to.06.output, "all_ORA_GO_objects.single_cell.rds"))
  saveRDS(go.ora, file.path(path.to.06.output, "all_ORA_GO_tables.single_cell.rds"))
} else {
  all.ORA.GO.objects <- readRDS(file.path(path.to.06.output, "all_ORA_GO_objects.single_cell.rds"))
  go.ora <- readRDS(file.path(path.to.06.output, "all_ORA_GO_tables.single_cell.rds"))
}
```
...
if (file.exists(file.path(path.to.06.output, "all_ORA_KEGG_objects.single_cell.rds")) == FALSE){
  kegg.ora <- hash()
  ...
  saveRDS(all.ORA.KEGG.objects, file.path(path.to.06.output, "all_ORA_KEGG_objects.single_cell.rds"))
  saveRDS(kegg.ora, file.path(path.to.06.output, "all_ORA_KEGG_tables.single_cell.rds"))
}else {
  all.ORA.KEGG.objects <- readRDS(file.path(path.to.06.output, "all_ORA_KEGG_objects.single_cell.rds"))
  kegg.ora <- readRDS(file.path(path.to.06.output, "all_ORA_KEGG_tables.single_cell.rds"))
}
...
if (file.exists(file.path(path.to.06.output, "GO_GSEA_all_GO_objects.single_cell.rds")) == FALSE | file.exists(file.path(path.to.06.output, "GO_GSEA_pathways_tables.single_cell.rds")) == FALSE){
  for(cluster.id in all.cluster.ids){
  ...
  saveRDS(object = go.gsea, file = file.path(path.to.06.output, "GO_GSEA_pathways_tables.single_cell.rds"))
  saveRDS(object = all.GSEA.GO.objects, file = file.path(path.to.06.output, "GO_GSEA_all_GO_objects.single_cell.rds"))
} else {
  go.gsea <- readRDS(file.path(path.to.06.output, "GO_GSEA_pathways_tables.single_cell.rds"))
  all.GSEA.GO.objects <- readRDS(file.path(path.to.06.output, "GO_GSEA_all_GO_objects.single_cell.rds"))
}
```
...
if (file.exists(file.path(path.to.06.output, "KEGG_GSEA_all_KEGG_objects.single_cell.rds")) == FALSE | file.exists(file.path(path.to.06.output, "KEGG_GSEA_pathways_tables.single_cell.rds")) == FALSE){
  for(cluster.id in all.cluster.ids){
    tmp.full.list <- de.markers.raw[[cluster.id]] %>% arrange(desc(avg_log2FC))
  ...
  saveRDS(object = kegg.gsea, file = file.path(path.to.06.output, "KEGG_GSEA_pathways_tables.single_cell.rds"))
  saveRDS(object = all.GSEA.KEGG.objects, file = file.path(path.to.06.output, "KEGG_GSEA_all_KEGG_objects.single_cell.rds"))
} else {
  kegg.gsea <- readRDS(file.path(path.to.06.output, "KEGG_GSEA_pathways_tables.single_cell.rds"))
  all.GSEA.KEGG.objects <- readRDS(file.path(path.to.06.output, "KEGG_GSEA_all_KEGG_objects.single_cell.rds"))
}
...
count.mtx.foxp3 <- GetAssayData(s.obj.full, slot = "counts", assay = "RNA")["Foxp3", ]

if (foxp3 == "positive"){
  foxp3.cells <- names(count.mtx.foxp3[count.mtx.foxp3 > 0])
  s.obj.full <- subset(s.obj.full, cells = foxp3.cells)
} else if (foxp3 == "zero") {
  foxp3.cells <- names(count.mtx.foxp3[count.mtx.foxp3 == 0])
  s.obj.full <- subset(s.obj.full, cells = foxp3.cells)
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
DimPlot(object = subset(s.obj.full, name %in% c(sample1, sample2)), reduction = "INTE_UMAP", label = TRUE, split.by = "name", ncol = 2)
```
...
- **Positive** `logFC` indicates that the gene is up in sample 2, `r params$sample2`.

- **Negative** `logFC` indicates that the gene is up in sample 1, `r params$sample1`.
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
writexl::write_xlsx(heatmap.full.input.scaled, file.path(path.to.06.output, sprintf("heatmap_input_%s_%s_%s_allDEGs.pseudobulk.scaled.xlsx", sample1, sample2, foxp3)))
```
...
## Over-representation analysis (ORA)
```{r echo=FALSE, warning=FALSE, results='asis', message=FALSE, include=FALSE}
library(clusterProfiler)
library(org.Mm.eg.db)
