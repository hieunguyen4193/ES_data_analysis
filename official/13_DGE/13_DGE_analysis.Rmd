...
path.to.save.output <- "/media/hieunguyen/CRC1382H/CRC1382/outdir/EStange_20240411_SeuratV5/data_analysis/13_output/all.samples/adult_SPF_d7_SPF"
  ...
  for (cluster.id in unique(s.obj$seurat_clusters)){
    ...
    if (count.sample1 >= 10 & count.sample2 >= 10){
      ...
      tmp.de.markers <- FindMarkers(object = tmp.s.obj, ident.1 = sample2, ident.2 = sample1, 
                                  group.by = "name", test.use = "wilcox", assay = "SCT",
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    p <- DoHeatmap(object = subset(s.obj, seurat_clusters == as.numeric(str_replace(cluster.id, "cluster_", ""))), features = c(top10up.genes$Gene, top10down.genes$Gene), group.by = "name", assay = "SCT")  
  }
  print(p)
  cat("\n \n")
}
...
count.mtx.foxp3 <- GetAssayData(s.obj.full, slot = "counts", assay = "SCT")["Foxp3", ]
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
writexl::write_xlsx(heatmap.full.input.scaled, file.path(path.to.save.output, sprintf("heatmap_input_%s_%s_%s_allDEGs.pseudobulk.scaled.xlsx", sample1, sample2, foxp3)))
```
