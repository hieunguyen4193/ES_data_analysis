...
# path.to.save.output <- "/media/hieunguyen/CRC1382H/CRC1382/outdir/EStange_20240411_reduced_RNAcontam_0/data_analysis/13_output/from_03_output/remove_d4_LPS_SC5/d7_SPF_d7_GF/part1"
...
if (file.exists(file.path(path.to.save.output, sprintf("%s.CHECK_FILE.csv", savename))) == FALSE){
  de.markers <- hash()
  de.markers.raw <- hash()
  ...
  write.csv(data.frame(status = c("finished saving")), file.path(path.to.save.output, sprintf("%s.CHECK_FILE.csv", savename)))
} else {
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    if (length( intersect(c(top10up.genes$Gene, top10down.genes$Gene), row.names(s.obj[["SCT"]]@scale.data)) ) > 0){
      p <- DoHeatmap(object = subset(s.obj, cca.cluster.0.5 == as.numeric(str_replace(cluster.id, "cluster_", ""))), 
                     features = c(top10up.genes$Gene, top10down.genes$Gene), group.by = "name", assay = "SCT") + 
...
if (file.exists(file.path(path.to.save.output, "ORA_GO.CHECK_FILE.csv")) == FALSE){
  go.ora <- hash()
  ...
  write.csv(data.frame(status = c("finished saving")), file.path(path.to.save.output, "ORA_GO.CHECK_FILE.csv"))
} else {
...
if (file.exists(file.path(path.to.save.output, "ORA_KEGG.CHECK_FILE.csv")) == FALSE){
  kegg.ora <- hash()
  ...
  write.csv(data.frame(status = c("finished saving")), file.path(path.to.save.output, "ORA_KEGG.CHECK_FILE.csv"))
}else {
...
if (file.exists(file.path(path.to.save.output, "GO_GSEA.CHECK_FILE.csv")) == FALSE){
  for(cluster.id in all.cluster.ids){
  ...
  write.csv(data.frame(status = c("finished saving")), file.path(path.to.save.output, "GO_GSEA.CHECK_FILE.csv"))
} else {
...
if (file.exists(file.path(path.to.save.output, "KEGG_GSEA.CHECK_FILE.csv")) == FALSE){
  for(cluster.id in all.cluster.ids){
    tmp.full.list <- de.markers.raw[[cluster.id]] %>% arrange(desc(avg_log2FC))
  ...
  write.csv(data.frame(status = c("finished saving")), file.path(path.to.save.output, "KEGG_GSEA.CHECK_FILE.csv"))
} else {
...
heatmap.full.input.scaled <- merge(heatmap.full.input.scaled, subset(resdf.sigOnly, select = c(Gene, padj, log2FoldChange, abslog2FoldChange)), by.x = "Gene", by.y = "Gene") %>% arrange(desc(log2FoldChange))
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
if (file.exists(file.path(path.to.save.output, sprintf("heatmap_input_%s_%s_allDEGs.pseudobulk.scaled.CHECK_FILE.xlsx", sample1, sample2))) == FALSE){
  ...
  ##### save CHECK FILE
  write.csv(data.frame(status = c("finished_saving")),
            file.path(path.to.save.output, sprintf("heatmap_input_%s_%s_allDEGs.pseudobulk.scaled.CHECK_FILE.xlsx", sample1, sample2)))
}
