...
if (file.exists(file.path(path.to.06.output, "monocle_obj.rds")) == FALSE){
  monocle.obj <- run_monocle2(s.obj, path.to.06.output)
} else {
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.06.output, "pseudotime.markers.rds")) == FALSE){
  pseudotime.markers <- differentialGeneTest(monocle.obj, fullModelFormulaStr = "~sm.ns(Pseudotime)", cores = 16, verbose = TRUE)
  saveRDS(pseudotime.markers, file.path(path.to.06.output, "pseudotime.markers.rds"))
} else {
  pseudotime.markers <- readRDS(file.path(path.to.06.output, "pseudotime.markers.rds"))
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
if (file.exists(file.path(path.to.06.output, "sce.rds")) == FALSE){
  ...
  saveRDS(sce, file.path(path.to.06.output, "sce.rds"))  
} else {
  sce <- readRDS(file.path(path.to.06.output, "sce.rds"))
}
```
...
##### Test within lineage
#####----------------------------------------------------------------------#####
if (file.exists(file.path(path.to.06.output, "tradeseq_test_res.rds")) == FALSE){
  ...
  saveRDS(tradeseq.testres, file.path(path.to.06.output, "tradeseq_test_res.rds"))
} else {
  tradeseq.testres <- readRDS(file.path(path.to.06.output, "tradeseq_test_res.rds"))
}
```
