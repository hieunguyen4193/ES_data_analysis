...
### Raw counts
```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE, fig.width=14, fig.height=10}
p <- count.cell.in.cluster %>% ggplot(aes(x = seurat_clusters, y = Freq, fill = HTO_classification)) + geom_bar(stat = "identity", dodge = "stack")
ggplotly(p)
```

```{r results='asis', echo=FALSE, message=FALSE, warning=FALSE, fig.width=14, fig.height=10}
p <- count.cell.in.cluster %>% ggplot(aes(x = HTO_classification, y = Freq, fill = seurat_clusters)) + geom_bar(stat = "identity", dodge = "stack")
ggplotly(p)
```
