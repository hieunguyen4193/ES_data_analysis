...
path.to.07.output <- file.path(path.to.main.output, "06_output")
...
if (file.exists(file.path(path.to.07.output, "monocle3.rds")) == FALSE){
  #####----------------------------------------------------------------------#####
  ...
  saveRDS(monocle.obj, file.path(path.to.07.output, "monocle3.rds"))
} else {
  monocle.obj <- readRDS(file.path(path.to.07.output, "monocle3.rds"))
}
...
p1 <- all.nodes %>% ggplot(aes(x = inteumap_1, y = inteumap_2, color = node.degree, label = label)) + geom_point(size = 3) + geom_text_repel(size = 6)
p2 <- all.nodes %>% ggplot(aes(x = inteumap_1, y = inteumap_2, color = node.degree, label = node.name)) + geom_point(size = 3) + geom_text_repel(size = 6)
p1 + p2
```
...
if (file.exists(file.path(path.to.07.output, "monocle.obj_graph_test_res.rds")) == FALSE){
  monocle.obj_graph_test_res <- graph_test(monocle.obj, neighbor_graph="principal_graph", cores=16)
  saveRDS(monocle.obj_graph_test_res, file.path(path.to.07.output, "monocle.obj_graph_test_res.rds"))
} else {
  monocle.obj_graph_test_res <- readRDS(file.path(path.to.07.output, "monocle.obj_graph_test_res.rds"))
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.height=10, fig.width=18}
gt0.5_genes <- subset(sig_monocle.obj_graph_test_res, sig_monocle.obj_graph_test_res$morans_I >= 0.5)$gene_name %>% head(100)
split.selected.genes <- split(gt0.5_genes, ceiling(seq_along(gt0.5_genes) / 10))
```
...
if (file.exists(file.path(path.to.07.output, "sce.rds")) == FALSE){
  ...
  #                               plot = TRUE,
  #                               parallel = FALSE)
  # saveRDS(eval_to_choose_K, file.path(path.to.07.output, "eval_to_choose_K.rds"))
