...
## Split by samples
```{r echo=FALSE, warning=FALSE, message=FALSE, results='hide', fig.width=28, fig.height=10}
DimPlot(object = s.obj, reduction = "INTE_UMAP", label = TRUE, label.box = TRUE, repel = TRUE, split.by = "name")
```
...
# Differential gene expression test between 2 samples
  ...
  for (cluster.id in unique(s.obj$seurat_clusters)){
    ...
    if (count.sample1 >= 10 & count.sample2 >= 10){
      tmp.de.markers <- FindMarkers(object = tmp.s.obj, 
                                  ...
                                  features = intersect(row.names(s.obj), genes.to.keep), recorrect_umi = FALSE)
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  if ("data" %in% colnames(inputdf) | nrow(inputdf) == 0){
    ...
    tryCatch(
        ...
        error=function(e) {
            p <- ggplot() + ggtitle("No data to generate heatmap for this cluster")
            return(p)
        }
    )
