...
s.obj.HTO <- CreateSeuratObject(counts = counts.adt[c("Hashtag1-TotalSeqC", "Hashtag2-TotalSeqC"), ], assay = "HTO")
...
**Total number of cells in this sample: 13575**
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
for (ht in c("Hashtag1-TotalSeqC", "Hashtag2-TotalSeqC")){
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
for (ht in c("CD44-TotalSeqC", "FR4-TotalSeqC", "CD73-TotalSeqC", "PD1-TotalSeqC")){
  hashtag.counts <- counts.adt[ht, ] %>% as.data.frame()
  colnames(hashtag.counts) <- c("count")
  cat(sprintf("### Hashtag: %s \n", ht))
  p <- ggplot(subset(hashtag.counts, hashtag.counts$count >= 0), aes(x = count)) + geom_histogram(bins = max(hashtag.counts$count)) + ggtitle(ht)  + 
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
for (ht in c("CD44-TotalSeqC", "FR4-TotalSeqC", "CD73-TotalSeqC", "PD1-TotalSeqC")){
  counts.adt.ht.others <- GetAssayData(s.obj.HTO.others, slot = "counts")
  hashtag.counts <- log2(counts.adt.ht.others)[ht, ] %>% as.data.frame()
  colnames(hashtag.counts) <- c("count")
  cat(sprintf("### Hashtag: %s \n", ht))
  p <- ggplot(subset(hashtag.counts, hashtag.counts$count >= 0), aes(x = count)) + geom_histogram(bins = 200) + ggtitle(ht)  + 
    theme(text = element_text(size=20), 
          axis.text.x = element_text(hjust=1, size = 20),
          axis.text.y = element_text(hjust=1, size = 20)) 
  print(p)
...
This section is devoted to the data analysis of the other Hashtags: 

## Assigned cut-off 
```{r echo=FALSE}
s.obj.HTO.others <- HTODemux(s.obj.HTO.others, assay = "HTO", positive.quantile = chosen.quantile.others)
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
sum.table <- table(s.obj.HTO.others$HTO_classification.global) %>% as.data.frame()
colnames(sum.table) <- c("assigned_group", "# cells")
sum.table %>% create_dt()
```
...
Note: `CD44-TotalSeqC_PD1-TotalSeqC`: This name indicates that the cell is a *doublet*; it is expressed in both `CD44-TotalSeqC` and `PD1-TotalSeqC`. 

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
detail.sum.table <- table(s.obj.HTO.others$HTO_classification) %>% as.data.frame()
colnames(detail.sum.table) <- c("Classified hashtag", "# cells")
detail.sum.table %>% create_dt()
```
...
## Illustration by a heatmap
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
HTOHeatmap(object = s.obj.HTO.others, assay = "HTO")
```
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=12, fig.height=12}
for (i in c(1, 2, 3, 4)){
  cat(sprintf("### Hashtag %s \n", rownames(s.obj.HTO.others[["HTO"]])[i]))
  p <- RidgePlot(s.obj.HTO.others, assay = "HTO", features = rownames(s.obj.HTO.others[["HTO"]])[i], ncol = 3)
  print(p)
  cat("\n \n")
}
```
...
for (i in c(1, 2, 3, 4)){
  cat(sprintf("### Hashtag %s \n", rownames(s.obj.HTO.others[["HTO"]])[i]))
  p <- RidgePlot(s.obj.HTO.others, assay = "HTO", features = rownames(s.obj.HTO.others[["HTO"]])[i], ncol = 3, slot = "counts")
  print(p)
  cat("\n \n")
}
```
