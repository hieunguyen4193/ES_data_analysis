---
title: "DEA between d4_LPS vs d4_SPF, sub-cluster `r params$sub.cluster.id`"
author:
...
if (file.exists(file.path(path.to.05.output, "high.ambientRNA.markers.rds")) == FALSE){
  ...
  saveRDS(high.ambientRNA.markers.raw, file.path(path.to.05.output, "high.ambientRNA.markers.raw.rds"))
  saveRDS(high.ambientRNA.markers, file.path(path.to.05.output, "high.ambientRNA.markers.rds"))
} else {
  high.ambientRNA.markers <- readRDS(file.path(path.to.05.output, "high.ambientRNA.markers.rds"))
  high.ambientRNA.markers.raw <- readRDS(file.path(path.to.05.output, "high.ambientRNA.markers.raw.rds"))
}
...
- **positive** `logFC` indicates that the gene is highly expressed in the first sample, `r params$sample1`.

- **negative** `logFC` indicates that the gene is highly expressed in the second sample, `r params$sample2`.

## Number of cells in each cluter, each sample
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
count.cluster.sample <- table(s.obj@meta.data %>% 
        subset(select = c(seurat_clusters, name))) %>% 
  as.data.frame()  %>%
  pivot_wider(names_from = "name", values_from = "Freq")
count.cluster.sample[[sprintf("pct_%s", sample1)]] <- count.cluster.sample[[sample1]]/sum(count.cluster.sample[[sample1]])
count.cluster.sample[[sprintf("pct_%s", sample2)]] <- count.cluster.sample[[sample2]]/sum(count.cluster.sample[[sample2]])

sum.count.clusters <- count.cluster.sample[[sample1]] + count.cluster.sample[[sample2]]

count.cluster.sample[[sprintf("pct_in_cluster_%s", sample1)]] <- count.cluster.sample[[sample1]]/sum.count.clusters
count.cluster.sample[[sprintf("pct_in_cluster_%s", sample2)]] <- count.cluster.sample[[sample2]]/sum.count.clusters

count.cluster.sample %>%  mutate_if(is.numeric, round, 6) %>% create_dt()
```

### Stacked bar plot: percentage of cells coming from each sample in each cluster
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
plotdf <- count.cluster.sample[, c("seurat_clusters", sprintf("pct_in_cluster_%s", sample1), sprintf("pct_in_cluster_%s", sample2))] %>%
  pivot_longer(!seurat_clusters, names_to = "Sample", values_to = "pct")

plotdf %>% ggplot(aes(x = seurat_clusters, y = pct, fill = Sample)) + geom_bar(stat = "identity")
```

## Number of cells in each sample (= number of cells in each pseudobulk sample)

```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
table(s.obj@meta.data %>% 
        subset(select = c(name))) %>% 
  as.data.frame() %>% create_dt()
```


```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=14, fig.height=10}
savename <- sprintf("sample_%s_vs_%s.rds", sample1, sample2)
savename.raw <- sprintf("sample_%s_vs_%s.raw.rds", sample1, sample2)
if (file.exists(file.path(path.to.05.output, savename)) == FALSE){
  ...
  for (cluster.id in unique(s.obj$seurat_clusters)){
    ...
    if (count.sample1 >= 10 & count.sample2 >= 10){
      tmp.de.markers <- FindMarkers(object = tmp.s.obj, ident.1 = sample1, ident.2 = sample2, 
...
for (cluster.id in all.clusters.in.de.markers){
  ...
  input.df <- de.markers.raw[[sprintf("cluster_%s", cluster.id)]] 
...
for (input.cluster in names(de.markers)){
  tmp.de.markers <- de.markers[[input.cluster]]
  cat(sprintf("### %s \n", input.cluster))
  if (nrow(tmp.de.markers) > 1) {
    top10up.genes <- tmp.de.markers %>% subset(p_val_adj <= 0.05 & avg_log2FC > 0) %>% arrange(desc(avg_log2FC)) %>% head(20)
    top10down.genes <- tmp.de.markers %>% subset(p_val_adj <= 0.05 & avg_log2FC < 0) %>% arrange(desc(avg_log2FC)) %>% tail(20)
    p <- DoHeatmap(object = subset(s.obj, seurat_clusters == str_replace(input.cluster, "cluster_", "")), 
                   features = c(top10up.genes$Gene, top10down.genes$Gene), 
                   group.by = "name", raster = TRUE) + scale_fill_gradientn(colours = rev(mapal))
  } else{
    p <- ggplot() + ggtitle("No DE gene to show on heatmap")  
  } 
  print(p)
  cat("\n \n")    
}
...
```{r echo=FALSE, warning=FALSE, message=FALSE, results='asis', fig.width=20, fig.height=12}
# writexl::write_xlsx(heatmap.full.input.scaled, file.path(path.to.05.output, sprintf("heatmap_input_%s_%s_%s_allDEGs.pseudobulk.scaled.xlsx", sample1, sample2, foxp3)))
```
