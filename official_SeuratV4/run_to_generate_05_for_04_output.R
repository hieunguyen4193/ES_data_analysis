...
for (output.index in names(all.path.to.s.obj)){
  for (celltype in names(all.path.to.s.obj[[output.index]])){
      path.to.s.obj <- all.path.to.s.obj[[output.index]][[celltype]]
      ...
      path.to.save.output <- file.path(path.to.05.output, output.index, celltype)
      output_dir <- file.path(path.to.save.html, output.index, celltype)
...
for (output.index in names(all.path.to.s.obj)){
  ...
  for (celltype in names(all.path.to.s.obj[[output.index]])){
      ...
      for (cluster.id in available.clusters){
        ...
        path.to.save.output <- file.path(path.to.05.output, output.index, celltype, cluster.id)
        output_dir <- file.path(path.to.save.html, output.index, celltype, cluster.id)
        dir.create(path.to.save.output, showWarnings = FALSE, recursive = TRUE)
        dir.create(output_dir, showWarnings = FALSE, recursive = TRUE)
        if (file.exists(file.path(output_dir, output_file)) == FALSE){
          rmarkdown::render(input = path.to.rmd, 
